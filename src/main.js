...
switch (os.platform()) {
	case "win32": {
		nut = require("./lib/win32/libnut.node");
		app.setAppUserModelId("com.dave-luhman.onecard-swiper");
		break;
	}
	case "darwin": {
		nut = require("./lib/darwin/libnut.node");
		app.dock.setIcon(path.join(__dirname, "img", "favicon-32.png"));
		break;
	}
	default:
		break;
...
const onSwipe = async (error, onecardData) => {
	...
	try {
		...
		mainWindow.webContents.send("error", `Entry error: ${sendError.message}`);
	}
};
